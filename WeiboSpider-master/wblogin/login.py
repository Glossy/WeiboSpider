...
def get_server_data(su, session):
    ...
    pre_data_res = session.get(prelogin_url, headers=headers) # 得到一个Response对象

    sever_data = eval(pre_data_res.content.decode("utf-8").replace("sinaSSOController.preloginCallBack", ''))
    # 将Response对象头的无效语句删除并将字符串声明为语句存储为一个字典
    return sever_data
...
def login_no_pincode(name, password, session, server_data):
    ...
    rs = get_redirect(name, data, post_url, session)    # rs : return_status 返回状态信息，若正常则为空值，异常则为对应异常信息
